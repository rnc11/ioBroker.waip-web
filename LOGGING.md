...
| `generateEinsatzMapImage` | Generating the incident map image failed as a whole (e.g. every tile failed, the maps directory couldn't be created, or writing the PNG failed) – `einsatz.kartenbildPfad` keeps its previous value; the incident itself is still processed normally, only the map image is missing/stale for this one. | `generateEinsatzMapImage: no tiles to fetch for lat=52.52 lon=13.405 zoom=16` |
